...
import { ResponseData } from './response-data.interface'; // Adjust the path as per your file structure
...
export class UserhomeComponent implements OnInit {
  ...
  backendurl = 'http://localhost:8086/citizens';
  ...
  fetchUsers() {
    this.http
      .get<ResponseData>(this.backendurl) // Use ResponseData as the type
      .pipe(
        map((responseData: ResponseData) => { // Use ResponseData as the type here as well
          const userArray: UserData[] = [];
          for (const key in responseData) {
            var x = { ...responseData[key] }; // Use responseData here instead of ResponseData
            userArray.push(x);
          }
          return userArray;
        })
